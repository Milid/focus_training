...
#buttons
grun, gquit, gcontinue, gpause, gchange_settings, gline, gspeed = None, None, None, None, None, None, None

#messages
title_msg, descr_msg, pick_line_msg,pick_speed_msg = None, None, None, None
...
def set_buttons():
  ...
  line_images = ["rose_5-4.png", "rose_6-7.png", "rose_7-8.png", "rose_9-10.png" ]
  line_coef = [(5,4),(6,7),(7,8),(9,10)]
  line_rect = (150,80,300,300)
  gline = GroupButton(line_rect, 2,2,4, 5, colors, images = line_images, values = line_coef)
  gline.set_group()
  speed_rect = (150, 430, 300, 50)
  fps_val = [15, 25,40]
  gspeed = GroupButton(speed_rect, 1,3,3, 5, colors, text = ["1", "2", "3"],values = fps_val)
  gspeed.set_group()
  ...
  gchange_settings = GButton(change_rect,runcolors, text = "Change settings",action = focus_intro)
...
def set_messages():
  ...
  descr_msg = Message(screen, "Just keep the mouse inside the yellow circle", 'freesansbold.ttf', 15,
                          (screen_width/2, 37), color['blue'])
  pick_line_msg = Message(screen, "Pick your line", 'freesansbold.ttf', 20,
                                      (screen_width/2, gline.group_rect[1] - 20), color['blue'])
  pick_speed_msg = Message(screen, "Pick your speed", 'freesansbold.ttf', 20,
                                  (screen_width/2, gspeed.group_rect[1] - 20 ), color['blue'])
...
def create_background(width = screen_width, height = screen_height, coef = default_coef):
  get_rose_curve(coef)
...
def focus_intro():
  ...
  while True:
      ...
      background, line_rect = create_background(screen_width, screen_height, coef )
